...
This file relates to the historic period for the Northern model.
...
# DO LAND COVER G1, PR_SUM AND RADIATION SEPARATELY
...
# Use 'chosen_bands' to specify any bands in particular
...
def create_image(year, month, selected_bands): # If necessary
    ...
    image = image.clip(final_north).updateMask(other)
...
for band in chosen_bands: # If necessary
    ...
    np.save(f'/gws/nopw/j04/bas_climate/users/clelland/model/training_north_all/training_north_e5l_0120_{band}_array.npy', final_band_array) # <-- Edit as necessary
