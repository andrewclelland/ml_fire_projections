"""
Northern model for making future projections under both CMIP6 models and all scenarios. Output will be saved as a netCDF file with prediction plots as png.
...
train_firecci_array = load_npy_file('/gws/nopw/j04/bas_climate/users/clelland/model/training_north_all/training_north_0123_fire_array.npy') # <-- Edit as necessary
...
# Load and process latitudes and longitudes - Edit as necessary
    ...
    for folder, scenario in zip(folders, scenarios):
        ...
        for year in years:
            for month in months:
                ...
                loaded_test_arrays = [
                    da.from_delayed(load_npy_file(f'/gws/nopw/j04/bas_climate/users/clelland/model/testing_{model}_north/single_north_{band}_array.npy'), shape=(1297169, 1), dtype=np.float32)
                    for band in constant_variables
                ] + [
                    da.from_delayed(load_npy_file(f'/gws/nopw/j04/bas_climate/users/clelland/model/testing_{model}_north/single_north_month_{month}_array.npy'), shape=(1297169, 1), dtype=np.float32)
                ] + [
                    da.from_delayed(load_npy_file(f'/gws/nopw/j04/bas_climate/users/clelland/model/testing_{model}_north/single_north_{band}_array.npy'), shape=(1297169, 1), dtype=np.float32)
                    for band in latlon_variables
                ] + [           
                    da.from_delayed(load_npy_file(f'/gws/nopw/j04/bas_climate/users/clelland/model/testing_{model}_north/{scenario}/testing_{model}_north_{scenario}_{year}_{month}_{band}_array.npy'), shape=(1297169, 1), dtype=np.float32)
                ...
                if np.any(y_prob):
                    ...
                    plt.title(f'XGBoost {model_long} {folder} North Predictions for {year}_{month:02d}')
