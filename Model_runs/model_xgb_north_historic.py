"""
Northern model for validation/testing on the historic period - adjust the years as required. Probability of burn to be considered for analysis - currently 50%.

When loading training data - the first line (currently hashed out) is the reduced training set used for validation, the second line (also currently hashed out) is the training set used for testing, and the third line contains all the training data for the future projections (including the additional 2021-23 period).
...
loaded_train_arrays = [
    #da.from_delayed(load_npy_file(f'/gws/nopw/j04/bas_climate/users/clelland/model/training_north/training_north_0120_{band}_array_reduced.npy'), shape=(264622476, 1), dtype=np.float32)
    #da.from_delayed(load_npy_file(f'/gws/nopw/j04/bas_climate/users/clelland/model/training_north_all/training_north_0120_{band}_array.npy'), shape=(311320560, 1), dtype=np.float32)
    ...
    #da.from_delayed(load_npy_file(f'/gws/nopw/j04/bas_climate/users/clelland/model/training_north/training_north_cems_0120_{band}_array_reduced.npy'), shape=(264622476, 1), dtype=np.float32)
    #da.from_delayed(load_npy_file(f'/gws/nopw/j04/bas_climate/users/clelland/model/training_north_all/training_north_cems_0120_{band}_array.npy'), shape=(311320560, 1), dtype=np.float32)
    ...
    #da.from_delayed(load_npy_file(f'/gws/nopw/j04/bas_climate/users/clelland/model/training_north/training_north_e5l_0120_{band}_array_reduced.npy'), shape=(264622476, 1), dtype=np.float32)
    #da.from_delayed(load_npy_file(f'/gws/nopw/j04/bas_climate/users/clelland/model/training_north_all/training_north_e5l_0120_{band}_array.npy'), shape=(311320560, 1), dtype=np.float32)
...
#train_firecci_array = load_npy_file('/gws/nopw/j04/bas_climate/users/clelland/model/training_north/training_north_0120_firecci_array_reduced.npy')
#train_firecci_array = load_npy_file('/gws/nopw/j04/bas_climate/users/clelland/model/training_north_all/training_north_0120_firecci_array.npy')
train_firecci_array = load_npy_file('/gws/nopw/j04/bas_climate/users/clelland/model/training_north_all/training_north_0123_fire_array.npy')
#y_train = da.from_delayed(train_firecci_array, shape=(264622476,), dtype=np.float32).ravel()
#y_train = da.from_delayed(train_firecci_array, shape=(311320560,), dtype=np.float32).ravel()
y_train = da.from_delayed(train_firecci_array, shape=(332688305,), dtype=np.float32).ravel()
...
for year in years:
    for month in months:
        ...
        loaded_test_arrays = [
            da.from_delayed(load_npy_file(f'/gws/nopw/j04/bas_climate/users/clelland/model/testing_north/testing_north_{year}_{month}_{band}_array.npy'), shape=(1297169, 1), dtype=np.float32)
            for band in top_band_names
        ] + [
            da.from_delayed(load_npy_file(f'/gws/nopw/j04/bas_climate/users/clelland/model/testing_north/testing_north_cems_{year}_{month}_{band}_array.npy'), shape=(1297169, 1), dtype=np.float32)
            for band in fwi_band_names
        ] + [
            da.from_delayed(load_npy_file(f'/gws/nopw/j04/bas_climate/users/clelland/model/testing_north/testing_north_e5l_{year}_{month}_{band}_array.npy'), shape=(1297169, 1), dtype=np.float32)
            for band in clim_band_names
        ]
        ...
        if np.any(y_test) or np.any(y_pred):
            ...
            plt.title(f"XGB North ROC curve for {year}_{month:02d}")
            plt.xlabel("False Positive Rate")
            plt.ylabel("True Positive Rate")
            plt.legend(loc="lower right")
            plt.grid()
            plt.savefig(f'xgb_north_{year}_{month:02d}_roc.png', dpi=300, bbox_inches='tight', transparent=True) # <-- Edit as necessary
            ...
            plt.title(f'Map Visualization of XGB North Predictions for {year}_{month:02d}')
            plt.xlabel('Longitude')
            plt.ylabel('Latitude')
            
            # Show the plot
            plt.savefig(f'/home/users/clelland/Model/preds_xgb_north_{year}_{month:02d}.png', dpi=300, bbox_inches='tight', transparent=True) # <-- Edit as necessary
            ...
            def compute_grid_percentages(pred_lons, pred_lats, actual_lons, actual_lats, grid_size_meters):
                ...
                lon_min = -179.9585113906597
                lon_max = 179.9585113906597
                lat_min = 45.654300689697266
                lat_max = 83.6434326171875
            ...
            preds_mask = np.array(final_dict['preds']) > 0.5 # <-- Edit prediction probability as necessary
            ...
            plt.savefig(f'/home/users/clelland/Model/xgb_north_{year}_{month:02d}_colour_grid.png', dpi=300, bbox_inches='tight', transparent=True) # <-- Edit as necessary
